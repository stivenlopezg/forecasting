...
fcasts_2013_wide %>%
    ...
    {
        traindata <- filter(aus_retail_2013_tr, State == .y$State)
        summarise_at(.x, vars(sdrift:ets_fixed), list(
            MAPE=function(x) MAPE(x - .x$Turnover, .x$Turnover),
            MASE=function(x) MASE(x - .x$Turnover, traindata$Turnover, .period=12, d=TRUE))
        )
    })
...
fcasts_2013_wide %>%
    group_modify(function(.x, .y)
    {
        traindata <- filter(aus_retail_2013_tr, Industry == .x$Industry[1])
        summarise_at(.x, vars(sdrift:ets_fixed), list(
            MAPE=function(x) MAPE(x - .x$Turnover, .x$Turnover),
            MASE=function(x) MASE(x - .x$Turnover, traindata$Turnover, .period=12, d=TRUE))
        )

---
title: "Using Tidyverts with the Australian retail data: introduction"
...
Tidyverts is designed to integrate with the [Tidyverse](https://tidyverse.org), the de facto standard for data analytics workflows in R (the similarity in names is intentional). In particular, it defines a new data structure called a _tsibble_, which is an extended version of a tibble for working with time series. You can use all the standard dplyr/purrr verbs with tsibbles.
...
- There is a very obvious increasing trend over time. The trend is also mostly monotonic, which reflects Australian economic conditions over this period (it was not as severely affected by the recession of 2001, and the global economic crisis of 2008, as many other developed nations).
...
The Tidyverts framework defines a number of other functions to help in exploratory analysis, in the fabletools package. For plotting, the `gg_subseries` function produces a seasonal subseries plot, which facets the time series by each season in the seasonal period:
...
The `gg_season` function produces a plot where the x-axis shows data from within each season. This plot type allows the underlying seasonal pattern to be seen more clearly, and can be useful in identifying years in which the pattern changes:

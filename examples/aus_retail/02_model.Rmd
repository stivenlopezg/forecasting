...
Let's examine the resulting output, for one time series. The plotted output from `autoplot` includes the point forecasts along with the 80% and 95% prediction intervals, for each model.
...
This shows that the `sdrift` model performs worst, which is expected given how simplistic it is. The `ets_auto` model performs best, probably because this particular dataset exhibits very clear trends and seasonal patterns.
